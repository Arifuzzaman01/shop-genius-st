  ...
  useEffect(() => {
    const fetchProduct = async () => {
      ...
      if (res?.data?.variant?.length > 0) {
        setSelectedAttributes(res.data.variant[0].attributes);
      }
    };
    fetchProduct();
  }, [id]);

  const allAttributeKeys = product?.variant
    ? Array.from(
        new Set(
          product.variant.flatMap((v) => Object.keys(v.attributes || {})),
        ),
      )
    : [];

  const findVariant = product?.variant?.find((v) =>
    Object.entries(selectedAttributes).every(
      ([key, value]) => v.attributes[key] === value,
    ),
  );

  const productImage = findVariant?.image || product?.imageURLs?.[0];
    ...
    <div className="max-w-6xl mx-auto">
      <div className=" bg-white border border-gray-300 rounded-2xl flex flex-col md:flex-row gap-6 my-5 ">
        ...
        <div className="p-5 w-full flex-3 space-y-5">
          ...
          {/* variant - Dynamic Attributes */}

          {allAttributeKeys.map((attrKey) => (
            <div key={attrKey} className="mb-4">
              <p className="font-semibold pb-1 capitalize">{attrKey}</p>
              {attrKey?.length > 1 && (
                <div className="flex justify-center md:justify-start gap-1.5">
                  {Array.from(
                    new Set(
                      product?.variant?.map((v) => v.attributes[attrKey]),
                    ),
                  ).map((val) => (
                    <button
                      key={val}
                      ...
                      className={`border px-3 py-1 rounded-sm transition-all ${
